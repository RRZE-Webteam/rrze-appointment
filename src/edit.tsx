...
function CalendarMultiSelect( {
	...
	const [ viewDate, setViewDate ] = useState(
		() =>
			new Date(
				latestSelectedDate
					? latestSelectedDate.getFullYear()
					: today.getFullYear(),
				latestSelectedDate
					? latestSelectedDate.getMonth()
					: today.getMonth(),
				1
			)
	);
...
function PreviewCalendar( {
	...
	const activeDateTitle = __( 'Times on %s', 'rrze-appointment' ).replace(
		'%s',
		...
		<Fragment>
						...
						{ hideWeekends ? (
							<Fragment>
								{ weekdays.slice( 0, 5 ).map( ( w ) => (
									<div
										className="rrze-appointment__weekday"
										key={ w }
									>
										{ w }
									</div>
			...
			{ activeDate && activeDateSlots.length > 0 && (
				<fieldset className="rrze-appointment__day-slots">
					<p className="rrze-appointment__day-slots-title">
						{ activeDateTitle }
					</p>
...
export default function Edit( { attributes, setAttributes }: EditProps ) {
	...
	const effectiveBreakDuration =
		...
		activeOverride.breakDuration !== undefined
			...
			: breakDuration;
	const applyRecurrence = ( nextRec: Recurrence ) => {
		if ( ! nextRec.freq || ! activeDate ) {
			...
			const nextDates = calendarDates.filter(
				( d ) => ! recDates.includes( d ) || d === recAnchor
			);
			setAttributes( {
				recurrence: {},
		...
		const manualDates = calendarDates.filter(
			( d ) => ! recDates.includes( d )
		);
		const nextDates = [
			...new Set( [ ...manualDates, ...expanded ] ),
		].sort();
		setAttributes( {
					...
					{ ! faudirError && faudirPersons.length > 0 && (
						<SelectControl
							...
							onChange={ ( value ) => {
								...
								if (
									person &&
									( person.consultationHours?.length ?? 0 ) >
										0
								) {
									setHoursOverlay( {
										person,
				...
				>
					...
					{ recFreq && (
						<TextControl
							...
							value={ recUntil }
							onChange={ ( value ) =>
								applyRecurrence( { ...rec, until: value } )
							}
						/>
					) }
				</PanelBody>
