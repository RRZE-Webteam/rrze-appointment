...
import { loadCurrentBooker, requestBooker, submitAppointment } from './api';
import { buildDateMap, parseSlotValue } from './slot-parser';
import {
	isBookingClosed,
	isBookingNotOpen,
...
export function initializeAppointmentForms(): void {
	...
	function initAppointmentForm(
		...
		function isSlotTooFarInAdvance( slotValue: string ): boolean {
			...
			return isBookingNotOpen( slotStart, new Date(), bookingMaxAdvance );
		}
		...
		function openOverlay(
			...
			function closeOverlay(): void {
				...
				const slotReplacement = Array.from(
					...
					( button ) =>
						button.dataset.slotValue === value && ! button.disabled
				);
				const fallback =
					slotReplacement ||
		...
		function renderDaySlots( date: string, focusTitle = false ): void {
			...
			if ( hasNotOpenSlots ) {
				...
				notice.className = 'rrze-appointment__booking-window-notice';
		...
		const onCorrectPage =
			! autoPage || autoPage === window.location.href.split( '#' )[ 0 ];
