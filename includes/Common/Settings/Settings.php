...
use RRZE\Appointment\Common\Settings\{
    Builder,
    Error,
    Flash,
    Option,
 ...
 *
 * @package RRZE\Appointment\Common\Settings
 */
class Settings
{
    /**
    ...
    public function styling()
    {
        ...
        echo '<style>.rrze-appointment-settings-error {color: #d63638; margin: 5px 0;}</style>';
    }
    ...
    {
        if (empty($this->tabs)) {
            $tab = $this->addTab(__('Unnamed tab', 'rrze-appointment'));
        } else {
            $tab = end($this->tabs);
        }
    ...
    public function save()
    {
        ...
        if (!current_user_can($this->capability)) {
            wp_die(__('You do not have enough permissions to do that.', 'rrze-appointment'));
        }
        ...
        $this->flash->set('success', __('Settings saved.', 'rrze-appointment'));
    }
